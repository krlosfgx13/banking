package com.project.banking.utils;

public abstract class CardConstants {
    public static final Long POINTS = 1L;
    public static final Long MILES = 2L;
    public static final double REWARD_VALUE = 0.06;
    public static final double DOLLAR_AMOUNT = 8;
    public static final double CLASSIC_CARD_REWARD_ACCUMULATION = 1;
    public static final double GOLD_CARD_REWARD_ACCUMULATION = 1.25;
    public static final double PLATINUM_CARD_REWARD_ACCUMULATION = 1.50;
    public static final double BLACK_CARD_REWARD_ACCUMULATION = 2;
    public static final String CLASSIC_CARD = "Classic";
    public static final String GOLD_CARD = "Gold";
    public static final String PLATINUM_CARD = "Platinum";
    public static final String BLACK_CARD = "Black";
}
